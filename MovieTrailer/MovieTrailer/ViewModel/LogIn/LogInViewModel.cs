...
using MovieTrailer.Base;
using MovieTrailer.Models;
using MovieTrailer.PCL.Common;
using MovieTrailer.PCL.Service;
using MovieTrailer.Validator;
using Newtonsoft.Json;
using Plugin.Toast;
using Plugin.Toast.Abstractions;
using Xamarin.Forms;

namespace MovieTrailer.ViewModels.LogIn
{
    public class LogInViewModel : ViewsModelBase, INotifyPropertyChanged
    {
        ...
        ICommand _UserLoginClicked;
        public ICommand UserLoginClicked =>
        _UserLoginClicked ?? (_UserLoginClicked = new Command(async () => await GetUserLoginAsync()));
        ...
        public async Task gotoLogInPage()
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    Device.BeginInvokeOnMainThread(async () =>
                    {
                        await Navigation.PushAsync(new MovieTrailer.Views.LogIn.LogIn());
                    });
                });
            }
            catch (Exception ex)
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    Device.BeginInvokeOnMainThread(async () =>
                    {
                        ...
                        //await Navigation.PushAsync(new MovieTrailer.Views.SignUp.SignUp());
                    });
                });
            }
            catch (Exception ex)
